...
class PlayerService
{
	...
	public function getListAllUserWithWinsPercentage(): array
	{
		...
		foreach($throws as $throw){
			$user_id = $throw->player_id;
			$countWins = Throws::where('user_id', '=', $throw->player_id)->where('dices_sum', 7)->count();
			$countTries = Throws::where('user_id', '=', $throw->player_id)->count();
			if($countTries != 0){
				$percentageWins = $countWins / $countTries;
			}else{
				$percentageWins = 0;
			}
			$user = User::find($throw->user_id);

			$resultsArr [$user_id] = array(
				'wins_perc' => $percentageWins,
				'user_name' => $user->name,
				'user_tries' => $countTries,
				'user_wins' => $countWins
